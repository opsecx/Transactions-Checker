...
<body>
	...
	<div>
		This tool lets you check various aspects around transactions. It's based on the indexer database, so results may
		be
		limited to what's decoded there.<br><br>
		Note that only wallets that have conducted at least one transfer will appear in this dataset, so the list is
		incomplete particularly when it comes to wallets with zero transactions.<br><br>

		We will be adding functions continously. Join our <a href="https://github.com/opsecx/Transactions-Checker/"
			target="_blank">github</a> for helping out!<br><br>
	</div>
	...
	<form action="<?php echo ($filename); ?>" method="get">
		...
		Sort by <select name="sort">
			...
			{
				echo (" selected");
			} ?>># Transactions Sent</option>
			<option value="3" <?php if ($sort === "3")
			{
				echo (" selected");
			} ?>># Transactions Received</option>
			<option value="4" <?php if ($sort === "4")
			{
				echo (" selected");
			} ?>>Amount Sent</option>
			<option value="5" <?php if ($sort === "5" or empty($sort))
			{
				echo (" selected");
			} ?>>Amount Received</option>
			<option value="6" <?php if ($sort === "6")
			{
				echo (" selected");
			} ?>># Wallets Outgoing</option>
			<option value="7" <?php if ($sort === "7")
			{
				echo (" selected");
			} ?>># Wallets Incoming</option>
		</select>
		<select name="ascdesc">
			<option value="asc" <?php if ($ascdesc === 'asc')
			{
				echo ('selected');
			} ?>>Ascending</option>
			<option value="desc" <?php if ($ascdesc === 'desc' or empty($ascdesc))
			{
				echo (' selected');
			} ?>>Descending
			</option>
		</select><br> From <input type="text" name="from" value="<?php echo ($from); ?>"> To <input type="text"
			name="to" value="<?php echo ($to); ?>">
		<select name="range">
			<option value="num_outgoing" <?php if ($range === 'num_outgoing')
			{
				echo (" selected");
			} ?>># Transactions
				Sent
			</option>
			<option value="num_incoming" <?php if ($range === 'num_incoming')
			{
				echo (" selected");
			} ?>># Transactions
				Received
			</option>
			<option value="sum_outgoing" <?php if ($range === 'sum_outgoing')
			{
				echo (" selected");
			} ?>>Amount Sent
			</option>
			<option value="sum_incoming" <?php if ($range === 'sum_incoming' or empty($range))
			{
				echo (" selected");
			} ?>>
				Amount
				Received</option>
			<option value="wallets_outgoing" <?php if ($range === 'wallets_outgoing' or empty($range))
			{
				echo (" selected");
			} ?>>#
				Wallets Outgoing</option>
			<option value="wallets_incoming" <?php if ($range === 'wallets_incoming' or empty($range))
			{
				echo (" selected");
			} ?>>#
				Wallets Incoming</option>
...
FULL JOIN (SELECT target as wallet, count(amount) as num_outgoing,
  ...
  ORDER BY 5 DESC";
	...
	elseif ($run_mode == 'num_transfers')
	{
		...
		if ($sort === '2')
		{
			$sort_o = "2 $ascdesc, 4 $ascdesc, 1 asc";
		}
		elseif ($sort === '3')
		{
			$sort_o = "3 $ascdesc, 5 $ascdesc, 1 asc";
		}
		elseif ($sort === '4')
		{
			$sort_o = "4 $ascdesc, 2 $ascdesc, 1 asc";
		}
		elseif ($sort === '5')
		{
			$sort_o = "5 $ascdesc, 3 $ascdesc, 1 asc";
		}
		elseif ($sort === '6')
		{
			$sort_o = "6 $ascdesc, 4 $ascdesc, 1 asc";
		}
		elseif ($sort === '7')
		{
			$sort_o = "7 $ascdesc, 5 $ascdesc, 1 asc";
		}
		else
		{
			$sort_o = "$sort $ascdesc";
